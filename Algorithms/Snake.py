...
def moveSnake():
    ...
    if positions[0][0] < 0:
        canvas.coords(snake[0],width-snakeSize,positions[0][1],width,positions[0][3])#not sure about width here
        canvas.pack()
    ...
    for i in range(1,len(snake)):
        if overlapping(sHeadPos, canvas.coords(snake[i])):
            gameOver = True
            canvas.create_text(width/2,height/2,fill="white",font="Times 20 italic bold", text="Game Over!")#check
    for i in range(1,len(snake)):
...
width = 750 # width of snake’s world
height = 750 # height of snake’s world
